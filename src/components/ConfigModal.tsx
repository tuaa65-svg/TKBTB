      ...
      <div className="bg-white border-2 border-black shadow-[6px_6px_0px_rgba(0,0,0,1)] w-full max-w-3xl overflow-hidden font-serif animate-in fade-in zoom-in-95 duration-150">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[82vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            {/* Special Needs Students Setting */}
            <div className="sm:col-span-2 p-3 bg-stone-50 border border-stone-300 space-y-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={Boolean(formData.hasSpecialNeedsStudent)}
                  onChange={(e) => setFormData({ ...formData, hasSpecialNeedsStudent: e.target.checked })}
                  className="w-4 h-4 text-black border-black rounded focus:ring-0 cursor-pointer"
                />
                <span className="text-xs font-bold text-black uppercase">
                  {isEn ? "Class has students with special needs / disabilities" : "Lớp có học sinh khuyết tật / hòa nhập"}
                </span>
              </label>
              <p className="text-[11px] text-stone-600 italic">
                {isEn 
                  ? "* Note: In accordance with official guidelines, only classes with disabled students will include the 'Objectives for students with disabilities' section in KHBD." 
                  : "* Lưu ý: Theo quy định, chỉ nêu mục Yêu cầu cần đạt đối với học sinh khuyết tật khi lớp thực tế có học sinh khuyết tật hòa nhập."}
              </p>
              {formData.hasSpecialNeedsStudent && (
                <div className="pt-1">
                  <label className="block text-[10px] uppercase font-bold text-black mb-1">
                    {isEn ? "Specific objective adjustments for students with disabilities:" : "Yêu cầu cần đạt điều chỉnh cho học sinh khuyết tật:"}
                  </label>
                  <input
                    type="text"
                    ...
                    placeholder={isEn 
                      ? "e.g., Participates in learning activities with peer support, grasps basic lesson concepts..." 
                      : "Ví dụ: Tham gia các hoạt động cùng bạn theo khả năng, nắm được nội dung cơ bản với sự hỗ trợ của GV và bạn bè."}
                    className="w-full px-3 py-1.5 text-xs border border-black bg-white focus:outline-none"
                  />
                </div>
              )}
            </div>
